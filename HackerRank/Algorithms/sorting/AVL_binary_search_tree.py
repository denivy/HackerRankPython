...
def CountAllKids(node):
    if node.leftChild is not None:
        CountAllKids(node.leftChild)
    if node.rightChild is not None:
        CountAllKids(node.rightChild)
    #print("initializing numKids counters")
    initKids(node)
...
def initKids(node):
    ...
    #print("node=",node)
    if node.leftChild is not None:
        node.numKids += initKids(node.leftChild)
    if node.rightChild is not None:
        node.numKids += initKids(node.rightChild)
    return node.numKids
...
def Rank(root,x):
    if root is None:
        ...
        return Rank(root.leftChild, x)
    elif x == root.key:
        if root.leftChild is not None:
            return root.leftChild.numKids + 1
        else:
            return 1
    elif x > root.key:
        if root.leftChild is not None:
            return root.leftChild.numKids + 1 + Rank(root.rightChild, x)
        else:
            return 1 + Rank(root.rightChild, x)
...
for _ in range(0,numTests):
    ...
    #array.sort()
    tree = AVLTree(array)
    print("numItems=",numItems,"len(array)=",len(array))    
    print("created a tree with", tree.elements_count,"elements")
    print("the tree has",tree.height() ,"levels")
    print("the biggest value is",tree.find_biggest(tree.rootNode))
    print("the smallest value is", tree.find_smallest(tree.rootNode))
    tree.sanity_check()
    CountAllKids(tree.rootNode)
    count=0
    #print(tree.inorder(tree.rootNode))
    for i in range(0,len(array)):
        ...
        while j > 0:
            ...
            if previous < key:
                count += 1
            #print("key=",key, "previous=",previous)            
            j -= 1
